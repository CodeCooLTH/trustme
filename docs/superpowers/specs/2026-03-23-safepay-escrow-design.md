...
[DELIVERED] → Buyer กดยืนยัน + อัพโหลดรูปหลักฐาน
       ↓
[COMPLETED] → ระบบปล่อยเงิน (mock payout) + เพิ่ม points ทั้งคู่
       ↓
(ถ้ามีปัญหา ตอน DELIVERED) → [DISPUTE] → Admin ตัดสิน → คืน Buyer หรือ ปล่อย Seller
```
...
**กฎ:** สถานะเดินหน้าอย่างเดียว ย้อนกลับไม่ได้ ยกเว้น DISPUTE ที่เปิดได้จาก DELIVERED

### Auto-Complete

- แอดมินตั้งค่า `confirm_deadline_days` (จำนวนวัน)
- เมื่อสถานะเป็น DELIVERED และเลย deadline → ระบบ auto-complete
- ใช้ `node-cron` ใน process เดียวกับ Next.js (MVP)
- รันทุกชั่วโมง: หา orders DELIVERED ที่เลย deadline → COMPLETED → เพิ่ม points → แจ้งเตือน
...
OrderStatus:     PENDING_PAYMENT | PAYMENT_UPLOADED | PAYMENT_RECEIVED |
                 SHIPPING | DELIVERED | COMPLETED | CANCELLED | DISPUTE
...
| confirmDeadline | DateTime? | Set เมื่อ DELIVERED |
| createdAt | DateTime | |
| updatedAt | DateTime | |
...
| Method | Endpoint | Auth | Description |
|--------|----------|------|-------------|
| GET | `/api/orders` | BUYER/SELLER | รายการ orders ของผู้ใช้ |
| GET | `/api/orders/[token]` | No | ดู order ผ่าน public token |
...
| Method | Endpoint | Auth | Description |
|--------|----------|------|-------------|
| POST | `/api/orders/[token]/dispute` | BUYER/SELLER | เปิด dispute |
...
| Method | Endpoint | Auth | Description |
|--------|----------|------|-------------|
| POST | `/api/upload` | Yes | อัพโหลดไฟล์ → `{ path: "/uploads/xxx.jpg" }` |
...
- Buyer ไม่สามารถเปิด dispute หลัง COMPLETED
...
| Rate limit | In-memory | MVP, ย้าย Redis ทีหลัง |
