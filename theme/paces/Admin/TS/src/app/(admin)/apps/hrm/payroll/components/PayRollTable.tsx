...
import { payrollData, PayrollType } from './data'
...
const PayRollTable = () => {
  ...
  const columns = [
    {
      id: 'select',
      header: ({ table }: { table: TableType<PayrollType> }) => <input type="checkbox" className="form-checkbox form-checkbox-light size-4.5" checked={table.getIsAllRowsSelected()} onChange={table.getToggleAllRowsSelectedHandler()} />,
      cell: ({ row }: { row: TableRow<PayrollType> }) => <input type="checkbox" className="form-checkbox form-checkbox-light size-4.5" checked={row.getIsSelected()} onChange={row.getToggleSelectedHandler()} />,
      enableSorting: false,
    ...
    columnHelper.accessor('actualSalary', { header: 'Actual Salary', cell: ({ row }) => <strong>{row.original.actualSalary}</strong> }),
    columnHelper.accessor('deductions', { header: 'Deductions' }),
    columnHelper.accessor('netPay', { header: 'Net Pay', cell: ({ row }) => <strong>{row.original.netPay}</strong> }),
...

    {
      ...
      cell: ({ row }: { row: TableRow<PayrollType> }) => (
        <div className="flex justify-center gap-1.5">
          ...
          <button className="btn btn-icon btn-sm border border-default-300 hover:border-default-400" onClick={() => setSelectedRowIds({ [row.id]: true })} data-hs-overlay="#confirm-delete-modal">
        ...
        <div className="card-header">
          ...
          <div className="flex flex-wrap items-center gap-2.5 md:flex-nowrap">
            ...
            <div className="input-icon-group">
              ...
              <select className="form-select" value={(table.getColumn('department')?.getFilterValue() as string) ?? 'All'} onChange={(e) => table.getColumn('department')?.setFilterValue(e.target.value === 'All' ? undefined : e.target.value)}>
            ...
            <div className="input-icon-group">
              ...
              <select className="form-select" value={(table.getColumn('date')?.getFilterValue() as string) ?? 'All'} onChange={(e) => table.getColumn('date')?.setFilterValue(e.target.value === 'All' ? undefined : e.target.value)}>
