...
import { departmentData, DepartmentType } from './data'
...
const DepartmentTable = () => {
  const columns = [
    {
      id: 'select',
      header: ({ table }: { table: TableType<DepartmentType> }) => <input type="checkbox" className="form-checkbox form-checkbox-light size-4.5" checked={table.getIsAllRowsSelected()} onChange={table.getToggleAllRowsSelectedHandler()} />,
      cell: ({ row }: { row: TableRow<DepartmentType> }) => <input type="checkbox" className="form-checkbox form-checkbox-light size-4.5" checked={row.getIsSelected()} onChange={row.getToggleSelectedHandler()} />,
      enableSorting: false,
