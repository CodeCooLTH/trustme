...
const EstimationsTable = () => {
  const columns = [
    {
      id: 'select',
      header: ({ table }: { table: TableType<EstimateType> }) => <input type="checkbox" className="form-checkbox form-checkbox-light size-4.5" checked={table.getIsAllRowsSelected()} onChange={table.getToggleAllRowsSelectedHandler()} />,
      cell: ({ row }: { row: TableRow<EstimateType> }) => <input type="checkbox" className="form-checkbox form-checkbox-light size-4.5" checked={row.getIsSelected()} onChange={row.getToggleSelectedHandler()} />,
      enableSorting: false,
      ...
      <div className="card-header">
        ...
        <div className="flex flex-wrap items-center gap-3 md:flex-nowrap">
          ...
          <div className="input-icon-group">
            ...
            <select value={(table.getColumn('value')?.getFilterValue() as string) ?? 'All'} onChange={(e) => table.getColumn('value')?.setFilterValue(e.target.value === 'All' ? undefined : e.target.value)} className="form-select">
