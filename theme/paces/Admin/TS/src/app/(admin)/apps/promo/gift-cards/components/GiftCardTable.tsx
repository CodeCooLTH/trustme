  ...
  const columns = [
    {
      id: 'select',
      size: 45,
      header: ({ table }: { table: TableType<GiftCardType> }) => <input type="checkbox" className="form-checkbox form-checkbox-light size-4.5" checked={table.getIsAllRowsSelected()} onChange={table.getToggleAllRowsSelectedHandler()} />,
      cell: ({ row }: { row: TableRow<GiftCardType> }) => <input type="checkbox" className="form-checkbox form-checkbox-light size-4.5" checked={row.getIsSelected()} onChange={row.getToggleSelectedHandler()} />,
    },
    ...
    columnHelper.accessor('status', {
      header: 'Status',
      cell: ({ row }) => (
        <span className={`fw-semibold flex items-center ${row.original.status === 'active' ? 'text-success' : row.original.status === 'redeemed' ? 'text-default-400' : row.original.status === 'expired' ? 'text-danger' : row.original.status === 'pending' ? 'text-warning' : ''}`}>
      ...
      <div className="card-header">
        ...
        <div className="flex flex-wrap items-center gap-3 md:flex-nowrap">
          ...
          <div className="input-icon-group">
            ...
            <select data-table-filter="gift-status" className="form-select" value={(table.getColumn('status')?.getFilterValue() as string) ?? 'All'} onChange={(e) => table.getColumn('status')?.setFilterValue(e.target.value === 'All' ? undefined : e.target.value)}>
          ...
          <div className="input-icon-group">
            ...
            <select data-table-filter="gift-type" className="form-select" value={(table.getColumn('type')?.getFilterValue() as string) ?? 'All'} onChange={(e) => table.getColumn('type')?.setFilterValue(e.target.value === 'All' ? undefined : e.target.value)}>
              ...
              <div className="card-body">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Flatpickr type="text" options={{ dateFormat: 'd M, Y', defaultDate: 'today' }} className="form-input flatpickr-input" name="issue_date" placeholder="Select issue date" required />
                  </div>
                  <div>
                    <label className="form-label">
                      Expiry Date
                    ...
                    <Flatpickr type="text" options={{ dateFormat: 'd M, Y', defaultDate: 'today' }} className="form-input flatpickr-input" name="expiry_date" placeholder="Select expiry date" required />
                  </div>
                  <div className="md:col-span-2">
