...
const TicketsTable = () => {
  const columns = [
    {
      id: 'select',
      header: ({ table }: { table: TableType<TicketType> }) => <input type="checkbox" className="form-checkbox form-checkbox-light size-4.5" checked={table.getIsAllRowsSelected()} onChange={table.getToggleAllRowsSelectedHandler()} />,
      cell: ({ row }: { row: TableRow<TicketType> }) => <input type="checkbox" className="form-checkbox form-checkbox-light size-4.5" checked={row.getIsSelected()} onChange={row.getToggleSelectedHandler()} />,
      enableSorting: false,
        ...
        <div className="card-header">
          ...
          <div className="flex flex-wrap items-center gap-3 md:flex-nowrap">
            ...
            <div className="input-icon-group">
              ...
              <select className="form-select" value={(table.getColumn('priority')?.getFilterValue() as string) ?? 'All'} onChange={(e) => table.getColumn('priority')?.setFilterValue(e.target.value === 'All' ? undefined : e.target.value)}>
