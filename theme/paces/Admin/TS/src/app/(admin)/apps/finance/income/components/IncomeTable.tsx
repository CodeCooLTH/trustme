...
import { incomeData, IncomeType } from './data'
  ...
  const columns = [
    {
      id: 'select',
      size: 45,
      header: ({ table }: { table: TableType<IncomeType> }) => <input type="checkbox" className="form-checkbox form-checkbox-light size-4.5" checked={table.getIsAllRowsSelected()} onChange={table.getToggleAllRowsSelectedHandler()} />,
      cell: ({ row }: { row: TableRow<IncomeType> }) => <input type="checkbox" className="form-checkbox form-checkbox-light size-4.5" checked={row.getIsSelected()} onChange={row.getToggleSelectedHandler()} />,
    },
  ...
  return (
    <>
      <div className="card">
        ...
        <DataTable<IncomeType> table={table} emptyMessage="No income found" />
