...
const AnalyticsOverview = () => {
  ...
  const columns = [
    {
      id: 'select',
      header: ({ table }: { table: TableType<AnalyticType> }) => <input type="checkbox" className="form-checkbox form-checkbox-light text-sm" checked={table.getIsAllRowsSelected()} onChange={table.getToggleAllRowsSelectedHandler()} />,
      cell: ({ row }: { row: TableRow<AnalyticType> }) => <input type="checkbox" className="form-checkbox form-checkbox-light" checked={row.getIsSelected()} onChange={row.getToggleSelectedHandler()} />,
      enableSorting: false,
  ...
  return (
    <div data-table data-table-rows-per-page={5} className="card">
      ...
      <DeleteConfirmationModal onConfirm={handleDelete} selectedCount={Object.keys(selectedRowIds).length} itemName="entries" />
    </div>
  )
}
