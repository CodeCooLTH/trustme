  ...
  return (
    <>
      <Header title="ดีลของฉัน" actions={
        <Link href="/seller/deals/create"><Button size="sm"><Plus className="h-4 w-4" /> สร้างดีลใหม่</Button></Link>
      } />
      <div className="p-4 lg:p-6">
        {deals.length === 0 ? (
          <EmptyState icon={Tag} title="ยังไม่มีดีล" description="สร้างดีลแรกของคุณ" action={
            <Link href="/seller/deals/create"><Button><Plus className="h-4 w-4" /> สร้างดีลแรก</Button></Link>
          } />
        ) : (
